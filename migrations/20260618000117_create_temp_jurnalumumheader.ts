...
export async function up(knex: Knex): Promise<void> {
  await knex.schema.raw(`CREATE TABLE [dbo].[temp_jurnalumumheader] (
  ...
  CONSTRAINT [PK_temp_jurnalumumheader] PRIMARY KEY ([id])
);`);
}

export async function down(knex: Knex): Promise<void> {
  await knex.schema.raw('DROP TABLE IF EXISTS [dbo].[temp_jurnalumumheader];');
}
