...
export async function up(knex: Knex): Promise<void> {
  await knex.schema.raw(`CREATE TABLE [dbo].[temp_jurnalumumheaderUNKNOWN] (
  ...
  CONSTRAINT [PK_temp_jurnalumumheaderUNKNOWN] PRIMARY KEY ([id])
);`);
}

export async function down(knex: Knex): Promise<void> {
  await knex.schema.raw('DROP TABLE IF EXISTS [dbo].[temp_jurnalumumheaderUNKNOWN];');
}
