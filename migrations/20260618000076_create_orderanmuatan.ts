...
export async function up(knex: Knex): Promise<void> {
  await knex.schema.raw(`CREATE TABLE [dbo].[orderanmuatan] (
  [orderan_id] varchar(200) NULL,
  ...
  CONSTRAINT [PK_orderanmuatan] PRIMARY KEY ([id])
);`);
}

export async function down(knex: Knex): Promise<void> {
  await knex.schema.raw('DROP TABLE IF EXISTS [dbo].[orderanmuatan];');
}
