...
export class TradoService {
  private readonly logger = new Logger(TradoService.name);
  // Nama tabel — dipakai untuk query SQL, redis key & logtrail namatabel.
  // Frontend membaca redis `trado-page-<n>`, jadi JANGAN diubah.
  private readonly tableName = 'trado';
  constructor(
  ...
  // Deteksi pelanggaran FOREIGN KEY MSSQL (error 547). trado.statusaktif
  // punya FK ke parameter, dan tabel transaksi punya FK ke trado.id.
  ...
  async create(createTradoDto: any, trx: any) {
    try {
      const {
        sortBy = 'nama',
        sortDirection = 'asc',
      ...
      const insertData = this.buildInsertData(createTradoDto, uuid);
      ...
      if (existingData) {
        ...
        const resultposition = await this.baseQuery(trx)
          .count('* as posisi')
          .where((qb) => {
            // PostgreSQL ORDER BY menaruh NULL di PALING BAWAH untuk ASC dan di
            // PALING ATAS untuk DESC — kebalikan MSSQL. Grid (findAll) memakai
            // orderBy default PG, jadi hitungan posisi harus ikut aturan itu:
            //   - ASC : baris NULL ada SETELAH baris baru → JANGAN dihitung.
            //   - DESC: baris NULL ada SEBELUM baris baru → HARUS dihitung.
            // (Sebelumnya terbalik mengikuti asumsi MSSQL NULL-first → posisi
            // overcount sebanyak jumlah baris NULL, mis. trado 15 nama NULL →
            // fokus setelah simpan meleset ke blok NULL di bawah.)
            if (dir === 'desc') {
      ...
      throw new Error(`Error creating trado: ${error.message}`);
    }
  }
  ...
  async findAll(
    ...
    try {
      ...
      this.logger.error('Error fetching trado data', error?.stack);
      throw new InternalServerErrorException('Failed to fetch trado data');
    }
  }
  ...
  async update(id: string, data: any, trx: any) {
    try {
      ...
      if (!existedData) {
        throw new Error('Trado not found');
      }
      ...
      console.error('Error updating trado:', error);
      if (error instanceof HttpException) throw error;
      if (this.isForeignKeyError(error)) {
        throw new ConflictException('Status Aktif yang dipilih tidak valid.');
      }
      throw new Error('Failed to update trado');
    }
  }
    ...
    try {
      ...
      if (this.isForeignKeyError(error)) {
        throw new ConflictException(
          'Trado tidak dapat dihapus karena sudah digunakan pada transaksi lain.',
        );
      }
  ...
  async exportToExcel(data: any[]) {
    ...
    worksheet.getCell('A2').value = 'LAPORAN TRADO';
