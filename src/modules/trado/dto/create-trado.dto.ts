...
export const CreateTradoSchema = z.object({
  nama: z
    .string()
    .trim()
    ...
    .refine(
      async (value) => {
        // Cek case-insensitive + trim: "TEST", "test", " test " dianggap sama.
        const exists = await isRecordExistCI('nama', value, 'trado');
        return !exists; // Validasi jika nama sudah ada
      },
      {
        message: 'Trado dengan dengan nama ini sudah ada',
      },
    ),
...
export type CreateTradoDto = z.infer<typeof CreateTradoSchema>;
