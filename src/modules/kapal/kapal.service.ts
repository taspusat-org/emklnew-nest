        ...
        query.where((qb) => {
          searchFields.forEach((field) => {
            if (['created_at', 'updated_at'].includes(field)) {
              ...
              qb.orWhere('p.memo', 'ilike', `%${sanitizedValue}%`);
            } else if (field === 'text') {
              qb.orWhere('p.text', 'ilike', `%${sanitizedValue}%`);
            } else if (field === 'pelayaran') {
              qb.orWhere('pelayaran.nama', 'ilike', `%${sanitizedValue}%`);
            } else {
        ...
        for (const [key, value] of Object.entries(filters)) {
          ...
          if (value) {
            if (key === 'created_at' || key === 'updated_at') {
              ...
              query.andWhere(`pelayaran.nama`, 'ilike', `%${sanitizedValue}%`);
