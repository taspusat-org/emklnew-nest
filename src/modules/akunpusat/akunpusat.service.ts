  ...
  async findAll(
    ...
    try {
      ...
      const applyFilters = (query) => {
        ...
        // Postgres: tanpa temp table — ambil baris yang coa-nya BUKAN parent
        // dari baris manapun (node non-parent). whereNotNull penting agar NULL
        // di subquery tidak membuat NOT IN mengembalikan kosong.
        if (isLookUp) {
          query = query.whereNotIn(
            'u.coa',
            trx.select('parent').from(this.tableName).whereNotNull('parent'),
          );
        }
        ...
        if (search) {
          ...
          query.where((builder) => {
            searchableFields.forEach((field) =>
              builder.orWhere(`u.${field}`, 'ilike', `%${sanitizedValue}%`),
            );
          });
        }
        ...
        Object.entries(filters).forEach(([key, value]) => {
          ...
          if (key === 'created_at' || key === 'updated_at') {
            ...
            query.andWhere(`t.nama`, 'ilike', `%${sanitizedValue}%`);
          } else if (key === 'statusaktif_nama') {
            query.andWhere(`p.text`, 'ilike', `%${sanitizedValue}%`);
