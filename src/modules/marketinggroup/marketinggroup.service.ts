    ...
    try {
      ...
      const query = trx(`${this.tableName} as mg`)
        .select([
          ...
          'mg.statusaktif',
          'mg.modifiedby',
          trx.raw("TO_CHAR(mg.created_at, 'DD-MM-YYYY HH24:MI:SS') as created_at"),
          trx.raw("TO_CHAR(mg.updated_at, 'DD-MM-YYYY HH24:MI:SS') as updated_at"),
        ...
        query.where((qb) => {
          searchFields.forEach((field) => {
            if (['created_at', 'updated_at'].includes(field)) {
              qb.orWhereRaw("TO_CHAR(mg.??, 'DD-MM-YYYY HH24:MI:SS') like ?", [
                field,
                `%${sanitizedValue}%`,
              ]);
        ...
        for (const [key, value] of Object.entries(filters)) {
          ...
          if (value) {
            if (key === 'created_at' || key === 'updated_at') {
              query.andWhereRaw("TO_CHAR(mg.??, 'DD-MM-YYYY HH24:MI:SS') LIKE ?", [
                key,
                `%${sanitizedValue}%`,
              ]);
