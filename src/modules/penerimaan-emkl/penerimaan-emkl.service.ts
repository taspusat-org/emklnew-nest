    ...
    try {
      ...
      if (search) {
        ...
        query.where((builder) => {
          builder
            ...
            .orWhere('coadebet.keterangancoa', 'ilike', `%${sanitizedValue}%`)
            .orWhere('coakredit.keterangancoa', 'ilike', `%${sanitizedValue}%`)
            .orWhere(
            ...
            .orWhere('coaproses.keterangancoa', 'ilike', `%${sanitizedValue}%`)
      ...
      if (sort?.sortBy && sort?.sortDirection) {
        if (sort?.sortBy === 'coadebet_text') {
          ...
          query.orderByRaw(
            `(${memoExpr} ->> 'MEMO') ${sort.sortDirection}`,
          );
