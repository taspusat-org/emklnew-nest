...
// 2. KHUSUS CREATE
// ------------------------
export const CreatePenerimaanEmklSchema = z
  .object({
    ...baseFields,
  ...
  .superRefine(async (data, ctx) => {
    ...
    const existsName = await isRecordExistCI('nama', data.nama, 'penerimaanemkl');
    if (existsName) {
      ctx.addIssue({
        path: ['nama'],
        code: 'custom',
        message: 'Penerimaan EMKL dengan nama ini sudah ada',
      });
    }
...
export type CreatePenerimaanEmklDto = z.infer<
  typeof CreatePenerimaanEmklSchema
>;
...
export type UpdatePenerimaanEmklDto = z.infer<
  typeof UpdatePenerimaanEmklSchema
>;
