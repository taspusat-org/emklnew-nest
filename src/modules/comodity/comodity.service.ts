...
export class ComodityService {
  private readonly logger = new Logger(ComodityService.name);
  // Nama tabel — dipakai untuk query SQL, redis key & logtrail namatabel.
  // Frontend membaca redis `comodity-page-<n>`, jadi JANGAN diubah.
  private readonly tableName = 'comodity';
  constructor(
  ...
  // Deteksi pelanggaran FOREIGN KEY MSSQL (error 547). comodity.statusaktif
  // punya FK ke parameter, dan tabel transaksi (hargatrucking.comodity_id)
  // punya FK ke comodity.id. Tanpa ini, FK violation jadi 500 generik tanpa
  // pesan yang jelas.
  ...
  private buildInsertData(dto: any, uuid?: string): Record<string, any> {
    // Hanya keterangan yang di-uppercase. rate adalah nilai numerik (disimpan
    // apa adanya) & statusaktif adalah id parameter (varchar UUID) — JANGAN
    // diubah-ubah casing-nya.
    return {
  ...
  async create(createComodityDto: any, trx: any) {
    try {
      const {
        sortBy = 'keterangan',
        sortDirection = 'asc',
      ...
      const insertData = this.buildInsertData(createComodityDto, uuid);
      ...
      if (existingData) {
        ...
        const resultposition = await this.baseQuery(trx)
          ...
          .where((qb) => {
            ...
            // pada hitungan ascending. (DESC: NULL di paling bawah, jadi cukup
            // `>=`.)
            if (dir === 'desc') {
      ...
      throw new Error(`Error creating comodity: ${error.message}`);
    }
  }
  ...
  async findAll(
    ...
    try {
      ...
      this.logger.error('Error fetching comodity data', error?.stack);
      throw new InternalServerErrorException('Failed to fetch comodity data');
    }
  }
  ...
  async update(id: string, data: any, trx: any) {
    try {
      ...
      if (!existedData) {
        throw new Error('Comodity not found');
      }
      ...
      // Bangun payload update — uppercase hanya keterangan. JANGAN ikut menulis
      // created_at (akan reset tanggal dibuat setiap edit).
      ...
      console.error('Error updating comodity:', error);
      if (error instanceof HttpException) throw error;
      if (this.isForeignKeyError(error)) {
        throw new ConflictException('Status Aktif yang dipilih tidak valid.');
      }
      throw new Error('Failed to update comodity');
    }
  }
    ...
    try {
      ...
      if (this.isForeignKeyError(error)) {
        throw new ConflictException(
          'Comodity tidak dapat dihapus karena sudah digunakan pada transaksi lain.',
        );
      }
  ...
  async exportToExcel(data: any[]) {
    ...
    worksheet.getCell('A2').value = 'LAPORAN COMODITY';
    ...
    const headers = ['NO.', 'KETERANGAN', 'RATE', 'STATUS AKTIF'];
    ...
    data.forEach((row, rowIndex) => {
      ...
      const rowValues = [rowIndex + 1, row.keterangan, row.rate, row.text];
    ...
    const tempFilePath = path.resolve(
      tempDir,
      `laporan_comodity_${Date.now()}.xlsx`,
    );
...
// fix posisi NULL-first diterapkan ke seluruh modul page-N (2026-06-24)
