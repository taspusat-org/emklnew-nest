...
export class LabaRugiKalkulasiController {
  ...
  async update(
    @Param('id') id: string,
    @Body(new InjectMethodPipe('update')) rawData: any,
    @Req()
    req,
  ) {
    // 1. Gabungkan ID dan jalankan validasi Zod secara manual di sini
    const pipe = new ZodValidationPipe(UpdateLabaRugiKalkulasiSchema);
    const data = (await pipe.transform(
      { ...rawData, id },
      { type: 'body', metatype: Object, data: '' },
    )) as UpdateLabaRugiKalkulasiDto;
