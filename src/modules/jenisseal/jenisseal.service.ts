...
export class JenissealService {
  private readonly logger = new Logger(JenissealService.name);
  // Nama tabel — dipakai untuk query SQL, redis key & logtrail namatabel.
  // Frontend membaca redis `jenisseal-page-<n>`, jadi JANGAN diubah.
  ...
  // Deteksi pelanggaran FOREIGN KEY MSSQL (error 547). jenisseal.statusaktif
  // punya FK ke parameter, dan tabel transaksi punya FK ke jenisseal.id.
  ...
  async create(createJenissealDto: any, trx: any) {
    try {
      ...
      const insertData = this.buildInsertData(createJenissealDto, uuid);
  ...
  async findAll(
    ...
    try {
      ...
      this.logger.error('Error fetching jenisseal data', error?.stack);
      throw new InternalServerErrorException('Failed to fetch jenisseal data');
    }
  }
    ...
    try {
      ...
      if (this.isForeignKeyError(error)) {
        throw new ConflictException(
          'Jenis Seal tidak dapat dihapus karena sudah digunakan pada transaksi lain.',
        );
      }
