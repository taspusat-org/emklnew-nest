...
export class JenisMuatanService {
  private readonly logger = new Logger(JenisMuatanService.name);
  // Nama tabel — dipakai untuk query SQL, redis key & logtrail namatabel.
  // Frontend membaca redis `jenismuatan-page-<n>`, jadi JANGAN diubah.
  ...
  // Deteksi pelanggaran FOREIGN KEY MSSQL (error 547). jenismuatan.statusaktif
  // punya FK ke parameter, dan tabel transaksi punya FK ke jenismuatan.id.
  ...
  async create(createJenisMuatanDto: any, trx: any) {
    try {
      ...
      const insertData = this.buildInsertData(createJenisMuatanDto, uuid);
      ...
      throw new Error(`Error creating jenis muatan: ${error.message}`);
    }
  }
  ...
  async findAll(
    ...
    try {
      ...
      this.logger.error('Error fetching jenismuatan data', error?.stack);
      throw new InternalServerErrorException('Failed to fetch jenismuatan data');
    }
  }
    ...
    try {
      ...
      if (this.isForeignKeyError(error)) {
        throw new ConflictException(
          'Jenis Muatan tidak dapat dihapus karena sudah digunakan pada transaksi lain.',
        );
      }
