...
export class DivisiService {
  private readonly tableName = 'divisi';
  constructor(
  ...
  async create(createDivisiDto: any, trx: any) {
    try {
      const {
        sortBy,
        sortDirection,
      ...
      throw new Error(`Error creating divisi: ${error.message}`);
    }
  }
  ...
  async exportToExcel(data: any[]) {
    ...
    worksheet.getCell('A2').value = 'LAPORAN DIVISI';
