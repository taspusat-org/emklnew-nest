...
@Injectable()
export class JenisbiayamarketingService {
  constructor(
  ...
  async create(createEmklDto: any, trx: any) {
    try {
      ...
      await this.logTrailService.create(
        {
          ...
          postingdari: 'ADD JENIS BIAYA MARKETING',
          idtrans: newItem.id,
      ...
      throw new Error(`Error creating Jenis Biaya Marketing: ${error.message}`);
    }
  }
  ...
  async findAll(
    ...
    try {
      ...
      const query = trx(`${this.tableName} as jbm`)
        .select([
          'jbm.id',
          'jbm.nama',
          'jbm.keterangan',
          ...
          'jbm.modifiedby',
          trx.raw(
            "TO_CHAR(jbm.created_at, 'DD-MM-YYYY HH24:MI:SS') as created_at",
          ),
          trx.raw(
            "TO_CHAR(jbm.updated_at, 'DD-MM-YYYY HH24:MI:SS') as updated_at",
          ),
        ...
        query.where((qb) => {
          searchFields.forEach((field) => {
            if (['created_at', 'updated_at'].includes(field)) {
              qb.orWhereRaw("TO_CHAR(jbm.??, 'DD-MM-YYYY HH24:MI:SS') like ?", [
                field,
                `%${sanitizedValue}%`,
              ]);
            } else {
        ...
        for (const [key, value] of Object.entries(filters)) {
          ...
          if (value) {
            if (key === 'created_at' || key === 'updated_at') {
              query.andWhereRaw(
                "TO_CHAR(jbm.??, 'DD-MM-YYYY HH24:MI:SS') LIKE ?",
                [key, `%${sanitizedValue}%`],
              );
            } else if (key === 'statusaktif_text') {
              query.andWhere(`statusaktif.text`, '=', sanitizedValue);
            } else {
              query.andWhere(`jbm.${key}`, 'like', `%${sanitizedValue}%`);
            }
          }
        }
      }
  ...
  async update(dataId: string, data: any, trx: any) {
    try {
      ...
      if (!existingData) {
        throw new Error('Jenis Biaya Marketing not found');
      }
      ...
      await this.logTrailService.create(
        {
          namatabel: this.tableName,
          postingdari: 'EDIT JENIS BIAYA MARKETING',
          idtrans: dataId,
          nobuktitrans: dataId,
          aksi: 'EDIT',
      ...
      console.error('Error updating jenis biaya marketing:', error);
      throw new Error('Failed to update jenis biaya marketing');
    }
  }
  ...
  async delete(id: string, trx: any, modifiedby: string) {
    try {
      ...
      await this.logTrailService.create(
        {
          namatabel: this.tableName,
          postingdari: 'DELETE JENIS BIAYA MARKETING',
  ...
  async exportToExcel(data: any[]) {
    ...
    worksheet.getCell('A2').value = 'LAPORAN JENIS BIAYA MARKETING';
    ...
    const tempFilePath = path.resolve(
      tempDir,
      `laporan_jenisbiayamarketing${Date.now()}.xlsx`,
    );
