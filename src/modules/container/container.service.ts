...
export class ContainerService {
  ...
  // Frontend membaca redis `container-page-<n>`, jadi JANGAN diubah.
  ...
  // Deteksi pelanggaran FOREIGN KEY MSSQL (error 547). container.statusaktif
  // punya FK ke parameter, dan tabel transaksi punya FK ke container.id.
  ...
  async create(createContainerDto: any, trx: any) {
    try {
      ...
      const insertData = this.buildInsertData(createContainerDto, uuid);
  ...
  async findAll(
    ...
    try {
      ...
      this.logger.error('Error fetching container data', error?.stack);
      throw new InternalServerErrorException('Failed to fetch container data');
    }
  }
    ...
    try {
      ...
      if (this.isForeignKeyError(error)) {
        throw new ConflictException(
          'Container tidak dapat dihapus karena sudah digunakan pada transaksi lain.',
        );
      }
